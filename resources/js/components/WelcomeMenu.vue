	...
	<main class='row row__menu'>
		...
		<div class='row__board col-sm-10 offset-sm-1 col-lg-6 offset-lg-3 col-12 col-md-8 offset-md-2' v-if="menu">
			<div class='container-fluid' id='menu-order'>
				...
				class='row row__item align-items-center mb-5 mb-md-0'
				...
				<div class='text-center mt-3'>
                    <button v-if='order.start' class='btn btn-olive' data-target="#order-submit-modal" data-toggle='modal' type='button'><!--Termina Orden/-->Finish Order</button>
				</div>
			</div>
		</div>
	</main>
					...
					<form @submit='finishOrder'>
						<div class='card card-body'>
							<div class='text-center'>
								...
								<hr/>
                                -->
                                <p>Finish your order by pressing the "<!--Terminar/-->Submit" button at the end or press the "<!--Continuar/-->Continue" button to continue modifying your order.</p>
							</div>
							<div class='form-group row'>
                                <label for='name' class='col-md-4 col-form-label text-md-right'>Your Name<!--/Tu Nombre--></label>
								<div class='col-md-6'>
...
${{ item.price }} X {{ order.items[item.id] }} = ${{ (order.items[item.id]*item.price).toFixed(2) }}
							...
							<div class='row'>
								<div class='col-12 text-center'>
                                    ...
                                    <button type='submit' class='btn btn-success' :disabled="isFinishing"><!--Terminar/-->Submit</button>
								</div>
							</div>
						</div>
					</form>
